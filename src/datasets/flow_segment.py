...
def long_enough(segment, threshold) -> bool:
    ...
    packet_cnt = sum([len(flow) for flow in segment])
    return (packet_cnt / len(segment)) >= threshold
    ...
    for file in os.listdir('..'):
        if file.endswith('.pcap') and file not in processed_file:
            ...
            with open(output_file, 'a') as output:
                packet2vector(file, output, label, interval, threshold=10)
                output.flush()
